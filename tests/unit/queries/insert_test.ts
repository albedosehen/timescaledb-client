...
const mockInsertOperations = {
  ...
  buildInsertOhlcQuery(candle: Ohlc, options: InsertOptions): string {
    ...
    return `
      ...
      VALUES ('${candle.symbol}', '${candle.timestamp}', '1m', ${candle.open}, ${candle.high}, ${candle.low}, ${candle.close}, ${candle.volume || 'NULL'})
      ${upsertClause}
    `.trim()
  },
  ...
  buildBatchInsertOhlcQuery(candles: Ohlc[], options: InsertOptions): string {
    const values = candles.map(candle => 
      `('${candle.symbol}', '${candle.timestamp}', '1m', ${candle.open}, ${candle.high}, ${candle.low}, ${candle.close}, ${candle.volume || 'NULL'})`
    ).join(', ')
