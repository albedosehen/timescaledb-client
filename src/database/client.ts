...
import { createHealthChecker, HealthChecker } from './health.ts'
...
/**
 * Database result interface for type safety
 */
...
export interface DatabaseLayerConfig {
  ...
  /** Optional client options for connection pooling */
  readonly client?: ClientOptions
  /** Enable health checking and monitoring */
  ...
  readonly healthCheckConfig?: {
    ...
    /** Callback for errors during health checks */
    readonly onError?: (error: Error, context: string) => void
    /** Callback for warnings during health checks */
    readonly onWarning?: (warning: string, context: string) => void
  }
  /** Optional logger for debugging and monitoring */
  readonly logger?: Logger
  /** Optional factory for creating postgres instances */
  readonly postgresFactory?: PostgresFactory
}
...
export function createSimplePool(
  ...
  return createConnectionPool(connectionConfig, logger, clientOptions, postgresFactory)
}
...
export async function testConnection(
  ...
  const pool = createConnectionPool(connectionConfig, logger, { maxRetries: 1 }, postgresFactory)
  ...
  try {
    const result = await pool.query(async (sql) => {
      ...
      const extensions = allExtensions.map((ext) =>
        `${(ext as ExtensionResult).extname}@${(ext as ExtensionResult).extversion}`
      )
